...
A shell run-as $pkg sed -i /BANDWIDTH_SHARING_OPT_IN/d shared_prefs/${pkg}_preferences.xml 2>/dev/null
...
$S/ui.sh $d tap "^OK$" >/dev/null 2>&1 && sleep 1
tab 0 3; shot 01-nearby
tab 1 3; shot 02-sharing-optin
$S/ui.sh $d tap "$ACCEPT_RE" >/dev/null && sleep 5; shot 03-share
tab 2 3; shot 04-chat
tab 3 3; shot 05-profile
A shell input swipe $((w/2)) $((h*3/4)) $((w/2)) $((h/4)) 400; sleep 2
$S/ui.sh $d tap "$SETTINGS_RE" >/dev/null && sleep 3; shot 06-settings
